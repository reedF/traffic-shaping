-- setting nginx.conf in http phase: to load white and blacklist
-- init_by_lua_file lua/init.lua;
-- setting in http\server\location:
-- 		  access_by_lua '
...
local function checkIp(checkTag)
	local ip = ngx.var.remote_addr
	logger(ngx.INFO,"=========>ip:" .. ip)
	if checkTag == 0 then
		-- blacklist
		if ip_in_cidrs(ip, global_ip_blacklist) then
			return ngx.exit(ngx.HTTP_FORBIDDEN)
		end
	elseif checkTag == 1 then
		-- whitelist
		if not ip_in_cidrs(ip, global_ip_whitelist) then
			return ngx.exit(ngx.HTTP_FORBIDDEN)
		end
	else
		return
